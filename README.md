...
## Изначальный модератор

При первом старте auth-service автоматически создаёт учётную запись модератора (идемпотентно — повторные запуски её не дублируют):

| Поле | Значение |
|------|----------|
| Логин (email) | `moderator@accessible.city` |
| Пароль | `Moderator2026!` |

> Переопределяются переменными окружения `SEED_MODERATOR_EMAIL` и `SEED_MODERATOR_PASSWORD` (см. `.env` → auth-service). После первого входа смените пароль через страницу профиля (или `POST /api/auth/change-password`).

Модераторов и роли назначают только модераторы/админы (страница профиля → «Пользователи» или `PATCH /api/auth/users/:id/role`). При публичной регистрации роль всегда `user`.

## Роли и самостоятельное повышение

- **Активный пользователь (volunteer)** — может сам перейти из роли `user` (`POST /api/auth/self-promote` с `{"role":"volunteer"}`) и затем проверять точки на карте: в панели «На проверке» видит список `pending`-барьеров (на карте — жёлтые метки с «!») и может их **подтвердить** (`POST /api/barriers/{id}/confirm`) или **опровергнуть** (`POST /api/barriers/{id}/complaint` с указанием причины).
- **Заявка на модератора** — `user`/`volunteer` подают заявку (`POST /api/auth/apply` с `{"role":"moderator","comment":"..."}`). Заявки видят модераторы/админы: `GET /api/auth/applications`, `POST /api/auth/applications/{id}/approve` / `/reject` (после одобрения роль меняется сразу).
- **Модератор** — в очереди модерации (`/api/moderation/queue`) видит для каждой заявки барьер: координаты, описание-комментарий и приложенные фотографии (S3-пресайгн-URL), затем одобряет или отклоняет с комментарием.
- **Добавление барьера** — при клике по карте показываются координаты точки; форма создания позволяет указать их вручную и прикрепить фотографии (`POST /api/barriers/{id}/photos`, multipart).
...
| Путь | Сервис |
|------|--------|
| `/api/auth/*` | регистрация, логин, refresh, validate; профиль (GET/PATCH), смена пароля, список пользователей и назначение ролей (модератор/админ); самоповышение (self-promote), заявки на роли (apply/get/review) |
